...
2. 【重要】実装上の確認ポイント（二重加算の可能性）
コードを拝見して、1点非常に気になった部分があります。研究発表の前に確認・修正しておいた方が良いかもしれません。
先ほどの会話で、別のラッパー（SafetyShieldWrapper）のコメントには以下のように書かれていました。
self.safety_margin = safety_margin # 判定半径(0.2) + 余裕(0.15) つまり、デフォルト値の 0.35 の中には、すでにジャマーの当たり判定である 0.2 が含まれています。
しかし、今回ご提示いただいたAPFのコードでは以下のようになっています。
influence_radius = self.safety_margin + 0.2
もし safety_margin にデフォルトの 0.35 が入ってきた場合、ここでさらに 0.2 が足されるため、影響圏が 0.55 という非常に大きな値になってしまいます。 （ジャマーから 0.55 も離れている、かなり安全な位置からすでに斥力が発生して進路が曲げられ始めることになります）。
意図的な設計の場合: 「ポテンシャル法は早めに滑らかに避け始めたいから、意図的に影響圏を 0.55 と広く取っている」ということであれば全く問題ありません。
バグの場合: もし「0.35 の距離に入ったら斥力を発生させたい」という意図だった場合、この + 0.2 は二重加算になってしまっているため、influence_radius = self.safety_margin に修正する必要があります。

```

研究計画書
6の今後の展望を消して、3の院でやることに含める。マルチエージェントとかflockingとか。

スライド
時間がどれだけかかるかがわからない。図は仕上げておいてもよいと思う。

.md
斥力とか回避の発火条件とか式で書いといたほうがいい。