...
outputs\trajectory_20260519_1315.png

状態予測の詳細：
先輩のコードではラッパーを利用していた。ラッパーとはあとから値の境界値クリップやパラメータの上書きを付け足しながら実行する空クラスみたいなもの。ラッパーで「障害物（到達予測地点）に急接近した時に強くハンドルを切る」動きをさせる。結果、衝突せず比較的滑らかに回避行動をとりながら進めるようになっている。予測の中身は単純な直線運動前提の物理計算だと思われる。円軌道の際にはどうなっているのかわからないがレイヤーを区別しないと意味がない（円軌道だとわかったうえで予測すると意味がない）ので多分予測ロジック自体は一つだと思う。

今回で再現は無理そうなので、障害物を動かすのみまたは予測までやるだけとしたい。ラッパーは使わない方針で行った場合予測した後どうすればいいのかわからないので考えないといけない